		...
		<?php foreach($this->lang->all as $language){?>
			<div class="newAlbumLangContainer clearfix">
				...
				<div class="row">
					...
					<br />
					<?php echo CHtml::textArea('newAlbumDescription_'.$language, '', array('class'=>'inputNewAlbum cle'));?>
				</div>
			</div>
		<?php } ;?>
		</div>
		...
		<div class="row">
			<?php 
				if($onlyCollection && $this->levelAccess !== 2)
				{
					...
					echo CHtml::CheckBox('isCollection', false, array('class'=>'isCollection')).' '.$this->tr('isCollection');
			?>
		</div>
		</div>
	<?php endif;?>
