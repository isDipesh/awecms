...
class MMenuItems extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('menu_id, weight, parent_id, level, left, right, enabled, expanded', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>128),
			array('type', 'length', 'max'=>5),
			array('controller, action, var_name, var_id', 'length', 'max'=>50),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, menu_id, weight, parent_id, level, left, right, title, type, controller, enabled, expanded, action, var_name, var_id', 'safe', 'on'=>'search'),
		);
	}
